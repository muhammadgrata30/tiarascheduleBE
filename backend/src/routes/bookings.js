...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    cb(new Error('Only images (jpg, jpeg, png) or PDF are allowed'));
  }
});
...
router.post('/:id/payment-proof', authenticateToken, upload.single('proof'), async (req, res) => {
  const bookingId = req.params.id;
  const userId = req.user.id;

  if (!req.file) {
    return res.status(400).json({ error: 'Please upload a payment proof image' });
  }

  const dbPath = `/uploads/proofs/${req.file.filename}`;
